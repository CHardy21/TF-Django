...
def dashboard_colaborador(request):
    if not hasattr(request.user, 'role') or request.user.role != 'colaborador':
...
def exportar_estadisticas_csv(request):
    ...
    response['Content-Disposition'] = 'attachment; filename="estadisticas_colaborador.csv"'
    ...
    for post in posts:
        ...
        writer.writerow([post.titulo, post.fecha_creacion, post.vistas, comentarios, estado])
